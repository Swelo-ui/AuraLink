...
export default function PersonalWorkspace() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', overflow: 'hidden', background: '#0F0A1A' }}>

      {/* Header */}
      <div style={{ flexShrink: 0 }} className="px-4 py-3 border-b border-aura-border/50 bg-aura-panel/95 backdrop-blur-md flex items-center gap-3 z-10">
        <button
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-white font-bold text-base truncate">Personal Space</h2>
      ...
      {/* Mobile Tab Bar - fixed height, never shrinks */}
      <div style={{ flexShrink: 0, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', width: '100%' }} className="md:hidden bg-aura-panel border-b border-aura-border/50 p-1.5 gap-1">
        {TABS.map(tab => {
          const Icon = tab.icon;
            ...
            return (
              ...
              >
                ...
                {tab.label === 'Notes' ? 'Personal Notes' :
                  tab.label === 'Schedule' ? 'My Schedule' : 'My Vault'}
              </button>
            );
          })}
        </div>
